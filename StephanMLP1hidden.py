...
class NeuralNetwork(object):
    ...
    def __init__(self):
        ...
        self.W2_4 = np.random.randn(self.hiddensize, self.outputsize) # hidden to output layer weights
        np.clip(self.W2_4, -3, 3)
    ...
    def feedForward(self, X):
        ...
        self.A4 = np.matmul(self.A2, self.W2_4) + self.bias
        return self.A4
    ...
    def gradient_hidden_output(self, delta):
        gradient1 = delta * self.A2[0][0]
        gradient2 = delta * self.A2[0][1]
        return [[gradient1[0][0]], [gradient2[0][0]]]

    #Delta for L3 units
    # def delta_hidden_hidden(self, delta_output):
    #     summation_left_node = delta_output * self.W2_4[0]
    #     summation_right_node = delta_output * self.W2_4[1]
    ...
    #Delta for L2 units
    def delta_input_hidden(self, delta_output):
        summation_left_node = delta_output * self.W2_4[0]
        summation_right_node = delta_output * self.W2_4[1]
    ...
    def backward_pass(self, X, Y, estimated_output):
        ...
        gradient_output = self.gradient_hidden_output(delta_output) # W2_4 has shape 2 * 1
        ...
        delta_hidden_L2 = self.delta_input_hidden(delta_output) # W1_2 has shape 3 * 2
        gradient_hidden_L2 = self.gradient_input_hidden(delta_hidden_L2, X)
        self.W2_4 = self.W2_4 - (self.learning_rate * np.asarray(gradient_output))
        self.W2_4 = np.clip(self.W2_4, -2, 2)
